...
func buildMediaFields(ctx context.Context, client *whatsmeow.Client, msg *waE2E.Message, payload map[string]any) error {
	if audioMedia := msg.GetAudioMessage(); audioMedia != nil {
		if config.WhatsappAutoDownloadMedia {
			...
			if err != nil {
				logrus.Warnf("Failed to download audio, forwarding metadata only: %v", err)
	...
	if documentMedia := msg.GetDocumentMessage(); documentMedia != nil {
		if config.WhatsappAutoDownloadMedia {
			...
			if err != nil {
				logrus.Warnf("Failed to download document, forwarding metadata only: %v", err)
	...
	if imageMedia := msg.GetImageMessage(); imageMedia != nil {
		if config.WhatsappAutoDownloadMedia {
			...
			if err != nil {
				logrus.Warnf("Failed to download image, forwarding metadata only: %v", err)
	...
	if stickerMedia := msg.GetStickerMessage(); stickerMedia != nil {
		if config.WhatsappAutoDownloadMedia {
			...
			if err != nil {
				logrus.Warnf("Failed to download sticker, forwarding metadata only: %v", err)
	...
	if videoMedia := msg.GetVideoMessage(); videoMedia != nil {
		if config.WhatsappAutoDownloadMedia {
			...
			if err != nil {
				logrus.Warnf("Failed to download video, forwarding metadata only: %v", err)
	...
	if ptvMedia := msg.GetPtvMessage(); ptvMedia != nil {
		if config.WhatsappAutoDownloadMedia {
			...
			if err != nil {
				logrus.Warnf("Failed to download video note, forwarding metadata only: %v", err)
